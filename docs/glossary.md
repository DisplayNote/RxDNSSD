...
| **mDNSResponder** | Apple's open-source daemon/core implementing mDNS + DNS-SD. Vendored source lives under `dnssd/src/main/jni/mdnsresponder/` — see [[dnssd-module]]. |
...
| **`dnssd` module** | Lowest-level module: plain Java API (`DNSSD` interface) + native mDNSResponder core. No Rx dependency. Published as `com.github.andriydruk:dnssd` historically, now under `com.displaynote.dnssd` (see [[publishing]]). |
